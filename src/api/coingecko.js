...
let call = async (data) => {
    try {
        ...
        let tokensData = data.map(token => ({ ...token,  change24hs: (response[token.token].usd_24h_change).toFixed(1), usd: response[token.token].usd, value: (response[token.token].usd * token.amount).toFixed(2)}))
        return tokensData
